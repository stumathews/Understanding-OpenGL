...
{        
    ...
    // ====================================
    // Buffer for 1st vertex Attribute  //
    // =====================================
    ...
    // ====================================
    // Buffer for 2nd vertex Attribute  //
    // =====================================
    ...
    // This is the data that we are going to upload into the buffer object
    static const float vertexColorData[] = 
    {        
    ...
    dataStoreSize = sizeof(vertexColorData); // size of the allocation
    initialData = nullptr; /// no initial data (otherwise pointer to initial data)
    intendedUse = GL_MAP_WRITE_BIT;  // allow map for reading and writing to buffer
                       /* |
                       GL_DYNAMIC_STORAGE_BIT |
                       GL_MAP_READ_BIT |
                       GL_MAP_PERSISTENT_BIT |
    ...
    // Specify the buffer's data store parameters:
    glNamedBufferStorage(buffer2, dataStoreSize, initialData, intendedUse);
    // Use this if you want to refer to the buffer assoacited with a binding point/target -> glBufferStorage(GL_ARRAY_BUFFER, nullptr, intendedUse);

    // Bind buffer to context using the GL_ARRAY_BUFFER binding point - this suggests we are going to use the buffer for storing vertex data
    glBindBuffer(GL_ARRAY_BUFFER, buffer2); // note this buffer can be re-binded to another binding point/target
    ...
    // Upload data directly by writing to the mapped buffer... this time using a more surgical approach specifying
    // exactly where to write the data within the buffer
    ptr = glMapBufferRange(GL_ARRAY_BUFFER, 0, dataStoreSize, GL_MAP_WRITE_BIT);
    if(ptr)        
        memcpy(ptr, vertexColorData, sizeof(vertexColorData)); // copy our vertex data directly into the buffer on the GPU!
    else
        SayHello("Bad range");
    ...
    /// ========================
    /// 2nd Vertex Attribute
    /// ============================
    
    // Bind a vertex buffer to the VAO
    vertexBufferBinding = 13;
    vertexAttributeIndex = 1;
    glVertexArrayVertexBuffer(vao, vertexBufferBinding, buffer2, start_within_buffer, vertexSize);
